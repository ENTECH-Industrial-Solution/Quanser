...
def draw_camera_regions(img, width, height):
    ...
    # Total scaled_w = 1340. Segments: Rear-Right(160), Left(320), Front(320), Right(320), Rear-Left(160) + margins(10 each)
    segments = [
        ("REAR-R", 10, 170),
        ("LEFT", 180, 500),
        ("FRONT", 510, 830),
...
try:
    ...
    # Create a resizable window — user can freely drag edges to resize
    win_name = 'Physical QCar2 - 360 Vision (YOLOv8 Detection)'
    ...
    while True:
        ...
        if agent.connected:
            ...
            if recvFlag:
                ...
                # Run YOLOv8 General Object Detection (every frame or alternate frames for speed)
                if enable_yolo and myYolo is not None:
                    # Run YOLOv8 inference on full 360 view
                    # YOLOv8 can detect all 80 COCO classes (person, car, motorcycle, stop sign, traffic light, bicycle, etc.)
