        ...
        while True:
            if os.path.exists(config_path):
                ...
                if current_mtime != last_mtime:
                    ...
                    # รีเซ็ตและลบกำแพงเดิมทั้งหมดในฉาก
                    qlabs.destroy_all_spawned_actors()
                    time.sleep(0.3)

                    # Spawn กำแพงใหม่จากไฟล์คอนฟิก
...
def main():
    ...
    # หากรันด้วยแฟล็ก --watch ให้เข้าโหมด Live Auto-Reload ทำงานต่อเนื่องวนลูปเฝ้ามองไฟล์
    if args.watch:
        watch_and_auto_reload(qlabs, CONFIG_FILE)
        return

    # โหมดรันครั้งเดียว (One-shot Mode)
    ...
    print("💡 สามารถเปิดไฟล์ watch_wall_config.bat เพื่อให้ระบบอัปเดตกำแพงอัตโนมัติเมื่อกด Save ได้ครับ")
