...
# ==============================================================================
# QCar2 Virtual Teleoperation (Controller Only)
# สคริปต์สำหรับการบังคับและควบคุมรถ QCar2 ใน QLabs (แยกเฉพาะส่วนการควบคุม)
# ==============================================================================

def main():
    print("==================================================")
    print("   QCar2 Virtual Teleoperation (Controller Only)  ")
    print("==================================================")
    ...
    # 2. ผูกการเชื่อมต่อกับตัวรถ QCar2 (Actor Number = 1) โดยไม่ต้อง Spawn ใหม่
    car_id = 1
    myCar = QLabsQCar2(qlabs)
    myCar.actorNumber = car_id

    # พิกัดสำหรับฟังก์ชั่นรีเซ็ต
    ...
    print("--------------------------------------------------")
    print("💡 หากยังไม่มีตัวรถในฉาก กรุณารัน spawn_qcar2.py ก่อนนะครับ\n")
        ...
        while True:
            ...
            if keyboard.is_pressed('h') or keyboard.is_pressed('H'):
                if not h_key_pressed:
                    ...
                    print(f"\n[HEADLIGHTS] {'ON' if headlights else 'OFF'}")
                    h_key_pressed = True
            else:
                h_key_pressed = False
            ...
            if keyboard.is_pressed('r') or keyboard.is_pressed('R'):
                if not r_key_pressed:
                    print("\n[RESET] รีเซ็ตตำแหน่งรถกลับจุดเริ่มต้น...")
                    speed = 0.0
                    steering_deg = 0.0
