...
# - ปรับหน้าต่าง Resizable (ย่อ/ขยายขนาดหน้าต่างได้อิสระ)
# - ขยายระยะแผนที่ (Map Range) พร้อมระบบกด ซูมเข้า-ซูมออก
# ==============================================================================
...
def draw_lidar_map(angles, distances, map_size=800, max_range=12.0):
    ...
    # 1. วาดวงแหวนระยะทาง (Range Rings)
    num_rings = 4
    ...
    if angles is not None and distances is not None and len(distances) > 0:
        ...
        # แปลงพิกัด Polar เป็น Cartesian (x, y)
        ...
        for i in range(len(distances)):
            ...
            if 0 <= pt_x < map_size and 0 <= pt_y < map_size:
                if dist < 0.75:
                    color = (0, 0, 255)      # แดง: อันตรายใกล้มาก (< 0.75m)
                elif dist < 1.5:
                    color = (0, 165, 255)    # ส้ม: เตือน (< 1.5m)
                elif dist < 2.5:
                    color = (0, 255, 255)    # เหลือง: ปานกลาง (< 2.5m)
                else:
                    color = (0, 255, 120)    # เขียว: ปลอดภัย

                cv2.circle(img, (pt_x, pt_y), 3, color, -1)

    # 3. วาดตัวรถ QCar2 ตรงกลางแผนที่
    cv2.circle(img, center, 12, (0, 0, 255), -1)  # จุดศูนย์กลางรถ (สีแดง)
    cv2.arrowedLine(img, center, (center[0], center[1] - 30), (0, 255, 0), 2, tipLength=0.3)

    # 4. วาด Overlay ข้อมูลและข้อความสถานะ
...
def main():
    ...
    # 2. ผูกการเชื่อมต่อกับตัวรถ QCar2
    car_id = 1
    myCar = QLabsQCar2(qlabs)
    myCar.actorNumber = car_id

    # เริ่ม Thread อ่านข้อมูล LiDAR
    lidar_worker = QLabsLidarThread(myCar, num_samples=400)
    lidar_worker.start()

    window_name = "QCar2 Virtual - 2D LiDAR Viewer"
    # ตั้งค่าหน้าต่างแบบ Resizable ( WINDOW_NORMAL ) และตั้งขนาดเริ่มต้นกะทัดรัด ( 480x480 )
    cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
    cv2.resizeWindow(window_name, 480, 480)

    current_max_range = 36.0  # ขยายระยะทางแผนที่เริ่มต้นเป็น 12.0 เมตร
