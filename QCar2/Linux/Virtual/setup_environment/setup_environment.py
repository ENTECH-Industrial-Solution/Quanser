...
def setup_demo_environment(qlabs):
    ...
    env.set_time_of_day(14.0)          # ตั้งเวลาบ่าย 2 โมง (14:00 น.)
    ...
    # วาดวงกลมตัวระบุรถแม่ (Parenting BasicShape กับตัวรถ QCar2 #1)
    marker1 = QLabsBasicShape(qlabs)
    marker1.spawn_id_and_parent_with_relative_transform(
        actorNumber=101,
        location=[1.2, 0, 1.85],
        rotation=[0, 0, 0],
        scale=[0.6, 0.6, 0.03],
        configuration=QLabsBasicShape.CONFIG_CYLINDER, # ทรงกระบอกแบน
        parentClassID=QLabsQCar2.ID_QCAR,
        parentActorNumber=1,
        waitForConfirmation=True
    )
    marker1.set_material_properties(color=[0, 1, 0], roughness=0.5, metallic=False)
